...
hash-loss finiteness, inference path, causal non-leak; both co-trained and
decoupled (detach-reps) modes."""
import torch
...
for detach in (False, True):
    m = LSHBucketSSA(d, h, block=4, topk=4, sel_dim=32, n_rounds=8, n_buckets=16,
                     cap=8, gate=True, causal=True, learn_hash=True,
                     hash_detach_reps=detach).to(dev)
    ...
    print(f"[detach={detach}] TRAIN ok: hash_loss={hl.item():.4f} "
          f"R.grad_norm={m.R.grad.norm().item():.4f}")
    ...
    assert leak < 1e-4, f"CAUSAL LEAK {leak}"
    print(f"[detach={detach}] INFER ok, causal leak={leak:.2e}")
