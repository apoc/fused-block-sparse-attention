...
% ─═ 5. Centroid Routing ═══════════════════════════════════════════════════
\section{Centroid Routing: Linear Selection}\label{sec:centroid}
...
capacity limitation (45M params, 3000--5000 steps), not a sparse attention
...
\begin{itemize}[leftmargin=*,itemsep=2pt]
\item The block-pair selection remains $\Ocomplex(n_{\text{blk}}^2)$ -sub-quadratic
  in $N$ by the constant $B_s^2$, but not genuinely linear. Centroid routing
  is linear but accuracy-broken.
...
The centroid routing negative result, perfect retrieval recall but degraded
task accuracy, isolates representation quality under coarse routing as the
central obstacle to genuinely linear selection, distinct from the
routing-quality problem that InfoNCE resolves. We believe this distinction is
useful for future work on sub-quadratic selection: solving the routing
objective is necessary but not sufficient.
