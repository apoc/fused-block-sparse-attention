...
selection by $3$ to $4\times$ at matched budget; single-needle retrieval, however,
needs roughly twice that block budget.
...
\item A \textbf{frontier-model validation}: swapping the training-free selector into
  ...
  twice the perplexity budget, a gap traced partly to the selector's query pooling
  (\Cref{sec:donor}).
\end{itemize}
