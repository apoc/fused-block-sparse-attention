...
Across both regimes the message is consistent: the lossless refine fixes the
\emph{representation} failure that sinks centroids, but hard hashing gives the
selector no gradient on blocks it does not already collide with, so it does not
learn to route without supervision. Block-sparse's all-pairs top-$k$ scores
every block on every step and learns to route in every regime. Reliable
unsupervised linear selection remains open.
