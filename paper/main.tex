...
its unsupervised (gate-only) training is fragile, leaving reliable linear
selection without supervision an open problem.
...
\subsection{LSH Bucketing: Lossless Refine, Fragile Without Supervision}
\label{sec:lsh}
...
bucketing, instead prunes the \emph{search}: block representations are hashed by
...
known needle block, available only on synthetic MQAR. Under gate coupling alone
(the regime a real LM must use), LSH is fragile: gate-only recall is highly
sensitive to the cosine gate temperature (hit 0.09, 0.34, 0.02 at scale 5.66,
30, 50) and never reaches block-sparse's gate-only level (acc 0.96, hit 0.78),
while a too-large temperature destabilizes training. We do not have a clean
account of this gap. Making a lossless-refine linear selector train reliably
without supervision is, for us, an open problem.
...
(gate-only) training proved fragile. Reliable linear selection without needle
