...
speedup at long contexts, while maintaining within-4\% perplexity on a
real language model. The gate-coupled selector training recipe is simple,
effective, and requires no separate pre-training stage. The centroid routing
negative result, perfect recall but broken accuracy, identifies
representation quality under coarse routing as the open challenge for
genuinely linear selection.
