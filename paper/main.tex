...
selector recovers roughly half of the heuristic's loss); the distilled map fails to
capture it because mean-pooling the query and key blocks averages away the
per-block extremes that the $\max q\cdot k$ estimate keeps. Recovering the oracle's
headroom with a learned, richer-feature selector, rather than this pooled linear
map, remains open.
...
It does not measure downstream long-context tasks (needle-in-a-haystack, RULER) or
training with the swap, both of which are future work, as is a grouped-query,
