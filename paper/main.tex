...
selection by $3$ to $4\times$ at matched budget; single-needle retrieval needs
roughly twice that block budget under mean-query selection, a gap a one-line
last-token selector largely closes.
...
\item A \textbf{frontier-model validation}: swapping the training-free selector into
  ...
  distilled linear selector captures none of it; single-needle retrieval needs about
  twice the perplexity budget under mean-query selection, a gap a one-line last-token
  query reduction largely closes (\Cref{sec:donor}).
\end{itemize}
...
it, scored by greedy match on the answer span ($40$ needles per cell). The dense
model retrieves perfectly at every context (Table~\ref{tab:niah}), so the harness is
sound. With the block-mean selector, retrieval needs a budget well above the $6\%$ at
which perplexity stays within $1.6\%$: at 32k, top-32 ($13\%$) retrieves $0.85$ while
top-16 ($6\%$) reaches only $0.20$, and at 64k even top-32 ($6\%$) reaches $0.08$. It
beats random at matched budget ($0.40$ vs.\ $0.00$ at 16k top-16). So the block-mean
selector needs roughly twice the perplexity budget to retrieve.
Most of this floor is the selector's query pooling, not content. SelectMax scores
...
whereas the lone retrieval query token ranks it near 8 at every length: pooling
$B_s$ queries dilutes the one that retrieves. Selecting by each query block's last
token instead (a one-line change, and exactly the query an autoregressive decode step
issues) sharply improves retrieval at matched budget: 16k top-16 from $0.40$ to
$1.00$, 32k top-16 from $0.20$ to $0.68$, and 64k top-32 from $0.08$ to $0.78$ (the
$6\%$ perplexity budget); it does not fully close the floor at the most aggressive
long-context budget (64k top-16 reaches only $0.23$), consistent with the single-token
rank rather than a propagation limit. A
two-stage mean-then-max variant helps less, since the per-token max re-inflates
competitor blocks. The prefill block-mean, not content selection, thus sets most of
the retrieval floor, and single-token decode should largely avoid it. This is
expected rather than new: decode-time selectors already use the single current query
(Quest~\cite{tang2024quest}, whose min/max key bound we reuse) and trained block
selectors gate per token (MoBA~\cite{lu2025moba}, NSA~\cite{yuan2025nsa};
TokenSelect~\cite{wu2024tokenselect} likewise separates prefill chunk-averaged from
decode per-query selection); we only quantify what the prefill block-mean costs
retrieval in a training-free swap and show a one-line reversion recovers most of it.

\begin{table}[h]
\centering
\caption{Single-needle retrieval (exact match, $40$ needles per cell) for the
block-sparse swap. Each budget cell shows the block-mean selector / the last-token
selector; the dense model retrieves perfectly ($1.00$) at every context. Block
fractions for top-8/16/32 are $6/13/25\%$ at 16k, $3/6/13\%$ at 32k, $2/3/6\%$ at
64k; random-16 is the block-mean selector over random blocks. The block-mean
selector needs about $13\%$ of blocks; the last-token selector retrieves at the
perplexity budget (64k top-32, $6\%$: $0.78$ vs.\ $0.08$).}
\label{tab:niah}
\small
\begin{tabular}{lcccc}
\toprule
context & top-8 & top-16 & top-32 & random-16 \\
 & mean / last & mean / last & mean / last & mean \\
\midrule
16k & $0.03/0.70$ & $0.40/1.00$ & $0.68/1.00$ & $0.00$ \\
32k & $0.00/0.33$ & $0.20/0.68$ & $0.85/0.97$ & $0.20$ \\
64k & $0.00/0.00$ & $0.00/0.23$ & $0.08/0.78$ & $0.00$ \\
\bottomrule
\end{tabular}
\end{table}
...
\item NIAH on our from-scratch models needs more scale (the 45M scores zero); we
  measure retrieval instead on the 35B donor swap (\Cref{sec:donor}), where the
  usable budget depends strongly on the query reduction.
...
\bibitem{wu2024tokenselect}
W.~Wu, Z.~Pan, C.~Wang, L.~Chen, Y.~Bai, T.~Wang, K.~Fu, Z.~Wang, and H.~Xiong.
\newblock {TokenSelect}: Efficient long-context inference and length extrapolation
  for {LLMs} via dynamic token-level {KV} cache selection.
\newblock \emph{arXiv preprint} arXiv:2411.02886, 2024.
